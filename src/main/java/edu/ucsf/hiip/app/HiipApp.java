...
public class HiipApp extends AbstractMessageTransformer{
    ...
    public Object transformMessage(MuleMessage message, String outputEncoding) throws TransformerException {
    	...
    	sb.append("<th scope=\"col\" colspan=\"2\">").append("OAuth2 States for HIIP User: ");
        sb.append(message.getInvocationProperty("patientId").toString()).append("</th>");
    	sb.append("</tr>");
    	sb.append("<tr>");
    	...
    	String hiipPatientIdentifier = "";
		String hiipAPEXUserIdentifier = "";
		String hiipClientIdentifier = "";
		String hiipClientSecret = "";
		if (queryParms.isEmpty()){
	    	//hiipPatientIdentifier = message.getInvocationProperty("encryptedPatientId").toString();
			hiipPatientIdentifier = message.getInvocationProperty("patientId").toString();
			hiipAPEXUserIdentifier = message.getInvocationProperty("APeXUserId").toString();
			hiipClientIdentifier = message.getInvocationProperty("clientId").toString();
			hiipClientSecret = message.getInvocationProperty("clientSecret").toString();
		} else {
			//hiipPatientIdentifier = queryParms.get("1").toString().trim();
			hiipPatientIdentifier = message.getInvocationProperty("patientId").toString();
			hiipAPEXUserIdentifier = queryParms.get("2").toString().trim();
			hiipClientIdentifier = queryParms.get("3").toString().trim();
			hiipClientSecret = queryParms.get("4").toString().trim();
		}
		...
		for (Object hash : lstPartners){
			...
			sb.append("<tr><th scope=\"row\">").append(partnerName).append("</th>");
    		...
    		if (iOauth.intValue() == 1) {
    			...
    			if (found && iActive.intValue() == 1){
    				...
    				if (null == refreshToken){
        				//send to authorize
    					sb.append("<td><a href=\"").append(BaseUrl).append("authorize?0=").append(partnerCode).append("&1=").append(message.getPayload()).append("&2=").append(hiipAPEXUserIdentifier).append("&3=").append(hiipClientIdentifier).append("&4=").append(hiipClientSecret).append("\">");
    					...
    					//send to deeplink
    					sb.append("<td><a href=\"").append(BaseUrl).append("deeplink?0=").append(partnerCode).append("&1=").append(message.getPayload()).append("&2=").append(hiipAPEXUserIdentifier).append("&3=").append(hiipClientIdentifier).append("&4=").append(hiipClientSecret).append("\">");
    					sb.append("<div style=\"height:100%;width:100%\">deeplink</div></a></td>");
        			
    					//the Revoke button  			
    					sb.append("<td><button onclick=\"location.href='")
    					  .append(BaseUrl).append("revoke?0=").append(partnerCode).append("&1=").append(message.getPayload()).append("&2=").append(hiipAPEXUserIdentifier).append("&3=").append(hiipClientIdentifier).append("&4=").append(hiipClientSecret).append("'\">Revoke</button></td>");
    				}
        			...
        			sb.append("<td><a href=\"").append(BaseUrl).append("authorize?0=").append(partnerCode).append("&1=").append(hiipPatientIdentifier).append("&2=").append(hiipAPEXUserIdentifier).append("&3=").append(hiipClientIdentifier).append("&4=").append(hiipClientSecret).append("\">");
